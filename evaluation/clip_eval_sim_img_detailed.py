...
def main(args):
    ...
    real_img, _ = next(
        load_and_preprocess([real_img_path], preprocess, device, batch=1)
    )
    real_ftr = clip_model.module.encode_image(real_img)
    ...
    with accelerator.split_between_processes(gen_img_path_ls) as input:
        ...
        for batch_images, batch_paths in load_and_preprocess(
            input, preprocess, device, args.batch
        ):

            # Extract the gen image feature
            with torch.no_grad():
                gen_ftr = clip_model.module.encode_image(batch_images)
