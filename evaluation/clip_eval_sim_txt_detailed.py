...
def main(args):
    ...
    with torch.no_grad():
        for txt_idx, txt in tqdm(prompt_ls, desc="Scoring prompts"):
            ...
            gen_img, path_name = load_and_preprocess(
                gen_img_path_ls, preprocess, args.device
            )
            ...
            sim = (txt_ftr @ gen_ftr.T).squeeze().cpu()
            ...
            per_scores.extend(sim_list)
            per_names.extend(path_name)

            scores += sim.mean().item()
            score_num += 1

            del sim, txt_ftr, gen_ftr
            torch.cuda.empty_cache()

    avg_score = scores / score_num
    result = {
        "gen_img_path": args.gen_img_path,
        "model": args.clip_model,
        "avg_score": avg_score,
        "per_samples_score": per_scores,
        "per_samples_name": per_names,
    }
